...
;; Built-in C/C++ mode configuration
(use-package cc-mode
  :ensure nil
  :mode (("\\.c\\'" . c-mode)
         ("\\.h\\'" . c-mode)
         ...
         ("\\.hxx\\'" . c++-mode)
         ("\\.hh\\'" . c++-mode))
  :hook ((c-mode . c-setup-minor-modes)
         (c-mode . c-setup-keybindings)
         (c-mode . eglot-ensure)
         (c-mode . flymake-mode)
         (c++-mode . cpp-setup-minor-modes)
         (c++-mode . cpp-setup-keybindings)
         (c++-mode . eglot-ensure)
         (c++-mode . flymake-mode))
  :config
  (setq c-default-style "linux"
...
;; C minor modes setup
(defun c-setup-minor-modes ()
  "Enable helpful minor modes for C."
  (electric-indent-local-mode 1)
  (subword-mode 1)
  (hs-minor-mode 1)
  (flyspell-prog-mode)
  (c-setup-include-paths)
  (setq-local tab-width 4
              indent-tabs-mode nil))

;; C keybindings setup
(defun c-setup-keybindings ()
  "Setup C mode keybindings."
  (local-set-key (kbd "C-c C-c") 'c-compile)
  (local-set-key (kbd "C-c C-r") 'c-run)
  (local-set-key (kbd "C-c C-f") 'c-format-buffer)
  (local-set-key (kbd "C-c C-d") 'eglot-find-declaration)
  (local-set-key (kbd "C-c C-i") 'eglot-find-implementation))

;; C++ minor modes setup
(defun cpp-setup-minor-modes ()
  "Enable helpful minor modes for C++."
  (electric-indent-local-mode 1)
  (subword-mode 1)
  (hs-minor-mode 1)
  (flyspell-prog-mode)
  (cpp-setup-include-paths)
  (setq-local tab-width 4
              indent-tabs-mode nil))

;; C++ keybindings setup
(defun cpp-setup-keybindings ()
  "Setup C++ mode keybindings."
  (local-set-key (kbd "C-c C-c") 'cpp-compile)
  (local-set-key (kbd "C-c C-r") 'cpp-run)
  (local-set-key (kbd "C-c C-f") 'cpp-format-buffer)
  (local-set-key (kbd "C-c C-d") 'eglot-find-declaration)
  (local-set-key (kbd "C-c C-i") 'eglot-find-implementation)
  (local-set-key (kbd "C-c C-s") 'cpp-switch-header-source))

;; clangd configuration for C and C++
(with-eval-after-load 'eglot
  ...
  (defun c-eglot-workspace-config ()
    "Return C workspace configuration for clangd."
    '(:clangd
      (:fallbackFlags ["-std=c11" "-Wall" "-Wextra"])))
  
  ;; Configure clangd for C++
  (defun cpp-eglot-workspace-config ()
    "Return C++ workspace configuration for clangd."
    '(:clangd
              ...
              (cond
               ((derived-mode-p 'c-mode)
                (setq-local eglot-workspace-configuration
                           (c-eglot-workspace-config)))
               ((derived-mode-p 'c++-mode)
                (setq-local eglot-workspace-configuration
                           (cpp-eglot-workspace-config)))))))

;; Simple include path setup for C
(defun c-setup-include-paths ()
  "Setup basic include paths for current C project."
  (let* ((current-dir (if buffer-file-name
    ...
    (setq-local c-include-paths (list project-root include-dir src-dir))))

;; Simple include path setup for C++
(defun cpp-setup-include-paths ()
  "Setup basic include paths for current C++ project."
  (let* ((current-dir (if buffer-file-name
                          (file-name-directory buffer-file-name)
                        default-directory))
    ...
    ;; Store include paths for potential use by other tools
    (setq-local cpp-include-paths (list project-root include-dir src-dir))))

;; C utility functions
(defun c-compile ()
  "Compile current C file or project."
  (interactive)
  (save-buffer)
  (let* ((file (buffer-file-name))
         (makefile (locate-dominating-file default-directory "Makefile"))
         (project-root (or makefile default-directory)))
    (cond
     (makefile
      (let ((default-directory makefile))
        (compile "make")))
     (t
      (let* ((include-flags (c-get-simple-include-flags project-root))
...
(defun c-get-simple-include-flags (project-root)
  "Get simple include flags for C compilation."
...
(defun c-format-buffer ()
  "Format C buffer with clang-format if available."
  (interactive)
  (cond
   ...
   (t (message "No C formatter found. Install clang-format"))))
...
(defun cpp-compile ()
  ...
  (let* ((file (buffer-file-name))
    ...
    (cond
     ...
     (t
      (let* ((include-flags (cpp-get-simple-include-flags project-root))
...
(defun cpp-get-simple-include-flags (project-root)
  "Get simple include flags for C++ compilation."
  (let ((dirs (list "." "include" "src")))
    (mapconcat (lambda (dir)
                 (let ((full-path (expand-file-name dir project-root)))
...
(defun cpp-switch-header-source ()
